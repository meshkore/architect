...
export function TokenUnlockHost(): JSX.Element {
  return (
    <Show when={opts()} keyed>
      {(o) => (
        <Modal
          isOpen={true}
          onClose={(id) => { if (id === 'ok') void submit(); else dismiss(); }}
          title="Unlock cluster"
          subtitle={o.project.cluster_name ?? o.project.cluster_id ?? `localhost:${o.project.port}`}
          buttons={[
            { id: 'cancel', label: 'Cancel' },
            { id: 'ok', label: busy() ? 'Validating…' : 'Save & unlock', primary: true },
          ]}
        >
          <div class="space-y-3">
            <Show when={o.reason}>
