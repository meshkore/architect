/**
 * TasksList — TASKS subtab (M4.3). Flat list filtered by active
 * scope + status pills + search; click a row to expand meta inline
 * (matches V80 monolith). Live from `serverStore.allTasks()`.
 */

import { For, Show, createSignal, createMemo } from 'solid-js';
import { allTasks, type ServerTask } from '~/state/server';

const STATUSES = ['next', 'in_progress', 'active', 'done', 'backlog', 'blocked'] as const;
type Status = typeof STATUSES[number];

export default function TasksList(props: { moduleId: string | null }) {
  const [statusFilter, setStatusFilter] = createSignal<Status | 'all'>('all');
  const [query, setQuery] = createSignal('');

  const filtered = createMemo<ServerTask[]>(() => {
    const q = query().toLowerCase().trim();
    const st = statusFilter();
    return allTasks().filter((t) => {
      if (props.moduleId && (t.category ?? t.module) !== props.moduleId) return false;
  ...
  return (
    <section class="min-w-0">
      <div class="flex flex-wrap items-center gap-2 mb-4 sticky top-14 bg-gray-950/80 backdrop-blur-md py-3 -mt-3 z-20">
        <h2 class="text-sm font-mono uppercase tracking-wider text-gray-500">Tasks</h2>
        <input
          type="text"
          placeholder="Search…"
          value={query()}
      ...
      <Show when={filtered().length > 0} fallback={<EmptyState scoped={!!props.moduleId} />}>
        <ul class="space-y-2">
          <For each={filtered()}>
            {(t) => <TaskRow task={t} />}
          </For>
      ...
      <p class="text-xs text-gray-600 mt-6">
        ...
        <Show when={props.moduleId}> · scope: <span class="font-mono text-gray-400">{props.moduleId}</span></Show>
        {' '}· live from daemon
      </p>
    </section>
  );
    ...
    <li class="bg-gray-900/40 border border-gray-800/70 rounded-lg overflow-hidden hover:border-gray-700 transition-colors">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="text-xs text-gray-500 mt-0.5 font-mono">
            {props.task.id}
            <Show when={props.task.category ?? props.task.module}>
              <span class="text-gray-700"> · </span>{props.task.category ?? props.task.module}
            </Show>
            <Show when={props.task.priority}>
              <span class="text-gray-700"> · </span>{props.task.priority}
            </Show>
            <Show when={props.task.initiative}>
              <span class="text-gray-700"> · </span><span class="text-emerald-400/70">{props.task.initiative}</span>
            </Show>
          </div>
        </div>
      ...
      <Show when={open()}>
        <div class="px-4 pb-3 pt-1 border-t border-gray-800/60 text-xs text-gray-500 font-mono break-all">
          <For each={Object.entries(props.task).filter(([k]) => k !== 'body')}>
            {([k, v]) => (
...
function StatusBadge(props: { status: string }) {
  const cls = () => {
    switch (props.status) {
      ...
      case 'in_progress': case 'active': return 'bg-blue-500/15 text-blue-300 border-blue-500/30';
...
function EmptyState(props: { scoped: boolean }) {
  return (
    <div class="text-center py-20 text-gray-500">
      <p class="text-sm">No tasks match the current filter{props.scoped ? ' for this scope' : ''}.</p>
    </div>
  );
}
