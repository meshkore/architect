 ...
 */
export function attachEventBus(ws: DaemonWS, client: DaemonClient): () => void {
  const unsubscribe = ws.onAny((ev: DaemonEvent) => {
    ...
    if (SNAPSHOT_REFRESH_TYPES.has(t)) {
      void serverStore.refresh(client);
    }
  });
  log.debug('event-bus attached');
  return () => {
    unsubscribe();
    log.debug('event-bus detached');
  };
}
