...
DAEMON_VERSION = "py-1.30.4"  # early bundle marker for the version-watcher 8 KB range-fetch; canonical def inlined below.
    ...
    def _teamext_reap_stale(
        ...
        for rid, e in data.items():
            ...
            if str(e.get("started_at") or "") >= cutoff:
                continue
            e["status"] = "error"
            e["error"] = "orphaned by a daemon restart mid-turn (no watcher survived to finish it)"
            e["finished_at"] = _iso_now()
            try:
                self._teamext_broadcast("team.request.error", str(e.get("member") or ""), rid)
            except Exception:
                pass
        return data
